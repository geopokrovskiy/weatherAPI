...
public class JwtHandler {
    ...
    public Mono<VerificationResult> check(String accessToken) {
        return Mono.just(verify(accessToken)).onErrorResume(
                e -> Mono.error(new UnauthorizedException(e.getMessage())
                ));
    }
    ...
    private VerificationResult verify(String token) {
        ...
        if (expirationDate.before(new Date())) {
            throw new AuthException("Token expired", ErrorCodes.TOKEN_EXPIRED);
        }
